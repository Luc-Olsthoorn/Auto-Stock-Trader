...
const stonks = [
  { name: "VOO", desiredAssetAllocation: 0.52 },
  { name: "VEA", desiredAssetAllocation: 0.12 },
  { name: "BND", desiredAssetAllocation: 0.12 },
  { name: "VPL", desiredAssetAllocation: 0.12 },
  { name: "vgt", desiredAssetAllocation: 0.12 },
];
...
const calculatePortfolioSpread = (stonksData) => {
  ...
  let returnStonksData = stonksData.map((stonk) => {
    ...
    const currentAssetAllocation =
      (stonk.numberOfStocks * stonk.currentValue) / total;
    const assetAllocationSpread =
      currentAssetAllocation - stonk.desiredAssetAllocation;
    return {
      ...stonk,
...
const getLinearRegressionSpread = async (stonkName) => {
  await workAround();
  let data = await alpha.data.daily_adjusted(stonkName);
